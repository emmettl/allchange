...
Run transfer checks on Node 24 as CI does: gzip measurements can differ between Node releases. The opening JavaScript budget is 346 KiB, including direct airport marker/label picking with 44-pixel mouse and 56-pixel touch targets and the shared renderer's GPU vehicle sampling from `@motionstudies/three` alpha.16. The interchange pulse loads after interaction and has a separate 5 KiB JavaScript / 2 KiB CSS budget; shared dependencies remain counted in the opening. Browser checks verify that its renderer is absent at startup and fetched when the pulse opens.
