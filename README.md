...
Run transfer checks on Node 24 as CI does: gzip measurements can differ between Node releases. The opening JavaScript budget is 347 KiB, including direct airport marker/label picking with 44-pixel mouse and 56-pixel touch targets and the shared renderer's budgeted vehicle scheduler from `@motionstudies/three` alpha.17. The interchange pulse loads after interaction and has a separate 5 KiB JavaScript / 2 KiB CSS budget; shared dependencies remain counted in the opening. Browser checks verify that its renderer is absent at startup and fetched when the pulse opens.

CI builds once and tests that artifact on three parallel runners: two Chromium shards and one iPhone WebKit job, each with one worker to avoid software WebGL contention. Chromium shards divide individual tests, including those within large spec files. Frame cadence runs once on the WebKit runner after its functional tests. Deployment waits for all three runners. Each runner uploads its own `e2e-report-<project>-<shard>` artifact and lists wall time and the slowest tests in the Actions summary.
...
Run `npm run data:london:national-rail` to rebuild from cached/public sources. Node 24+, `unzip` and Poppler’s `pdftotext` are required; `RAIL_CACHE` overrides `/tmp/allchange-rail-complete`. The build stages the resulting `network-*.json` files. The [rail completion record](docs/RAIL-COMPLETION.md) describes coverage, source audits, validation and the reproducible pipeline.

This is a published timetable study with interpolated movement. Live Darwin data, temporary alterations, freight and empty stock are outside this phase. Eurostar now has a separate dated integration: **55 St Pancras calls and 40 supported London HS1 movements**, with the other 15 calls explicitly timetable only. See [Eurostar sources, clocks and coverage](docs/EUROSTAR.md); rebuild its pinned sources with `npm run data:london:eurostar`. A later server-side Darwin adapter can use the retained CRS, UID and originating-date identities.
...
Stratford, Liverpool Street, Clapham Junction, Paddington, Waterloo, Victoria, London Bridge and Euston have combined **TfL + National Rail** station boards. King’s Cross and St Pancras add four distinct rail-area boards, including Eurostar, selectable from their shared Tube hero. Explicit source identities join their station areas; per-source time ranges, partial coverage and independent retries remain visible. Service selection and **Show movement** retain the correct renderer and shared clock. See [combined-board coverage and validation](docs/COMBINED-STATION-BOARDS.md).
