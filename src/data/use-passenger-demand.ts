...
export function usePassengerDemand(name: string) {
  const [area, setArea] = useState<{ name: string; asc: string }>()
  const [attempt, setAttempt] = useState(0)
  const areaId = area?.name === name ? area.asc : undefined
  const requestKey = `${name}:${areaId ?? ''}:${attempt}`
  const [result, setResult] = useState<{ key: string; value?: Awaited<ReturnType<typeof loadPassengerSelection>>; failed?: boolean }>()
  useEffect(() => {
    let active = true
    loadPassengerSelection(name, areaId).then(value => { if (active) setResult({ key: requestKey, value }) }).catch(() => { if (active) setResult({ key: requestKey, failed: true }) })
    return () => { active = false }
  }, [name, areaId, requestKey])
