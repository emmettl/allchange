...
import { roadLabelAnchors, visibleRoadLabels } from './london-road-labels.ts'
...
describe('road labels across camera changes', () => {
  it('adds detail on zoom while retaining visible labels at exactly the same road positions', () => {
    const anchors = roadLabelAnchors('M25-A', 'M25', [new THREE.Vector3(-10, 0, 0), new THREE.Vector3(10, 0, 0)], 2)
  ...
  it('samples bends and short roads without duplicating anchors at zero-length segments', () => {
    const anchors = roadLabelAnchors('bend', 'M4', [
    ...
    expect(roadLabelAnchors('short', 'M1', [new THREE.Vector3(), new THREE.Vector3(1, 0, 0)], 2)[0].position.x).toBe(0.5)
    expect(roadLabelAnchors('empty', 'M1', [new THREE.Vector3(), new THREE.Vector3()], 2)).toEqual([])
  })
})
