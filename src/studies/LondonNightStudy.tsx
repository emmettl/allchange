...
export default function LondonNightStudy({ date, time, name, stopIds, onTime, onStation }: {
  ...
  if (stopIds) {
    ...
    return <section className="london-night" aria-label={`${name} night departures`}>
      ...
      {!summary.inside ? <p>Night view ends at 05:00.</p> : !summary.available ? <p>No audited night context for this station.</p> : <>
        ...
        <p>{summary.partial ? 'TfL rail’s preceding-day tail is incomplete; a timetable gap does not prove no service ran.' : 'Published timetable gaps, not live running or a guaranteed connection.'} Rail only; buses are separate. Passenger demand is unavailable before 05:00.</p>
      </>}
    </section>
  }
  ...
  return <section className="london-night" aria-label="After midnight">
    ...
    <p>TfL rail’s preceding-day tail is incomplete. Gaps on the map are not proof that a line was closed. Passenger demand before 05:00 is unavailable.</p>
    ...
    <details><summary>Sources and coverage</summary><p>TfL recurring bus schedules include Thursday’s overnight tail. National Rail retains dated working-timetable carry-in. TfL rail and PDF sources do not fully establish Thursday’s tail. The full-day source branch exclusions still apply. No Saturday service or NUMBAT demand is wrapped into Friday morning.</p><p>Sources: Transport for London (transport data terms); Network Rail working timetable. Geometry and interpolation are the existing study’s model; none of these counts is live.</p></details>
  </section>
}
