...
function intervalLabel(index: number, start = 18000) {
  const minutes = start / 60 + index * 15
...
export default function LondonPassengerDemand({ stationName, time, onPulse }: { stationName: string; time: number; onPulse?: () => void }) {
  const { selection, loading, failed, retry, selectArea } = usePassengerDemand(stationName, time >= 0 && time < 18000)
  ...
  const start = selection?.data.start ?? 18000
  const label = (index: number) => intervalLabel(index, start)
  const spacing = 336 / (values.length || 96)
  const peak = Math.max(1, ...values)
  const peakIndex = values.indexOf(Math.max(...values))
  const path = values.map((v, i) => `${i ? 'L' : 'M'}${(4 + (i + 0.5) * spacing).toFixed(1)},${(66 - v / peak * 58).toFixed(1)}`).join(' ')
  return <section className="london-passenger-demand" aria-label="Typical passenger demand">
    <header><strong>Passenger rhythm</strong><span>{start === 0 ? 'Thursday tail · typical Tue–Thu' : 'Typical Friday'} · autumn 2025</span></header>
    {!station ? <p role="status">{failed ? <>Passenger data unavailable. <button type="button" onClick={retry}>Retry passenger data</button></> : 'Loading passenger profile…'}</p> : <>
      ...
      <p className="passenger-interval">{interval === undefined ? 'No matching interval in this study day' : `${label(interval)} · estimated people / 15 min`}</p>
      ...
      <svg viewBox="0 0 344 90" role="img" aria-label={`${labels[metric]} daily profile. Peak ${label(peakIndex)}, approximately ${number(values[peakIndex])} people per 15 minutes.`}>
        {start !== 0 && <rect x="270" y="3" width="70" height="65" fill="#ffffff07" />}
        ...
        {interval !== undefined && <g data-testid="passenger-marker" data-interval={interval}>
          <line x1={4 + (interval + 0.5) * spacing} x2={4 + (interval + 0.5) * spacing} y1="3" y2="67" stroke="#eee9d7" strokeDasharray="2 3" />
          <circle cx={4 + (interval + 0.5) * spacing} cy={66 - values[interval] / peak * 58} r="3" fill={colours[metric]} />
        </g>}
        {start === 0 ? <g fill="#bdb8ca" fontSize="9"><text x="4" y="84">00 Fri</text><text x="170" y="84">02:30</text><text x="340" y="84" textAnchor="end">05 Fri</text></g> : <g fill="#bdb8ca" fontSize="9"><text x="4" y="84">05</text><text x="99" y="84">12</text><text x="183" y="84">18</text><text x="270" y="84">00</text><text x="340" y="84" textAnchor="end">05 Sat</text></g>}
      </svg>
      <p className="passenger-peak">{labels[metric]} peak: {label(peakIndex)} · ≈{number(values[peakIndex])}/15 min<br />≈{number(station.totals[metric]!)} across {start === 0 ? '00:00–05:00 Friday' : 'the traffic day'}</p>
      ...
      <details><summary>About this profile</summary>
        ...
        <p>{start === 0 ? 'Early Friday uses Thursday’s 24:00–29:00 tail from the typical Tuesday–Thursday workbook.' : 'Traffic day: Friday 05:00 to Saturday 05:00. The shaded tail belongs to Saturday.'} Timetable: September 2026. Demand: autumn 2025.</p>
        <a href={selection!.data.source.url} target="_blank" rel="noreferrer">Powered by TfL Open Data · NUMBAT 2025 {selection!.data.source.dayType}</a>
      </details>
    </>}
  </section>
}
