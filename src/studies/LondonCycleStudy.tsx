...
const dateLabel = (date: string) => new Intl.DateTimeFormat('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' }).format(new Date(`${date}T12:00:00Z`))
...
export default function LondonCycleStudy({ time, onTime, isPlaying, onPlaying, rate, onRate, onClose, geography, railStops }: {
  ...
  const [date, setDate] = useState<CycleDate>('2026-05-29')
  const [attempt, setAttempt] = useState(0)
  const [result, setResult] = useState<{ key: string; value?: Awaited<ReturnType<typeof loadCycleDay>>; failed?: boolean }>()
  const requestKey = `${date}:${attempt}`
  const current = result?.key === requestKey ? result : undefined
  const day = current?.value?.day, manifest = current?.value?.manifest, failed = current?.failed
  const [selectedDock, setSelectedDock] = useState<CycleDock>()
  ...
  useEffect(() => {
    let active = true
    loadCycleDay(date).then(value => { if (active) setResult({ key: requestKey, value }) }).catch(() => { if (active) setResult({ key: requestKey, failed: true }) })
    return () => { active = false }
  }, [date, requestKey])
  useEffect(() => {
  ...
  useEffect(() => {
    const keyboard = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') { setSelectedDock(undefined); setQuery(''); setZoom(1) }
    }
  ...
  useEffect(() => {
    ...
    if (!element || !day || !indexed || !manifest) return
    const draw = () => {
      ...
      if (dock) label(dock.name.split(',')[0], ...xy(dock.lon, dock.lat))
  ...
  }, [day, manifest, indexed, geography, active, interval, time, dock, selected, zoom, reduced, interchanges])
  ...
  return <main className={`cycle-study london-experience${day && dock && comparisonOpen && !dismissed ? ' is-comparing' : ''}`} data-reduced-motion={reduced}>
    ...
    <div className="cycle-toolbar">
      <div className="cycle-search"><input ref={search} disabled={!day} type="search" role="combobox" aria-label="Find a docking station" aria-expanded={choices.length > 0} aria-controls="cycle-results" aria-activedescendant={choices[choice] ? `cycle-choice-${choice}` : undefined} placeholder="Find a docking station…" value={query} onChange={event => { setQuery(event.target.value); setChoice(0) }} onKeyDown={event => {
        ...
        if (event.key === 'Enter' && choices[choice]) { event.preventDefault(); selectDock(choices[choice]) }
    ...
    {!day ? <section className="cycle-loading" role="status">{dateLabel(date)}: {failed ? <>Cycle study unavailable. <button onClick={() => setAttempt(n => n + 1)}>Retry cycle study</button></> : 'Loading the cycle-hire day…'}</section> : <div className="cycle-content">
      <section className="cycle-map" aria-label="Cycle-hire geography">
        <canvas ref={canvas} role="img" aria-label={`${missingDock ? 'Selected dock unavailable on this date' : `${number(active.length)} recorded hires in progress${dock ? ` involving ${dock.name}` : ''}`}. Straight dock connections; positions are schematic. Use the station search to select a dock.`} onClick={event => {
          ...
          if (closest >= 0) selectDock(day.stations[closest])
        }} />
        <div className="cycle-map-tools"><button aria-label="Zoom in on cycle map" disabled={zoom >= 4} onClick={() => setZoom(z => Math.min(4, z * 1.5))}>+</button><button aria-label="Zoom out on cycle map" disabled={zoom <= 1} onClick={() => setZoom(z => Math.max(1, z / 1.5))}>−</button><button onClick={() => { setZoom(1); setSelectedDock(undefined); setDismissed(false) }}>All docks</button></div>
        <span className="cycle-map-count">{missingDock ? 'Dock unavailable this day' : `${number(active.length)} hires in progress`}</span>
      ...
      <aside className="cycle-card" data-hero-dismissed={dismissed} aria-label={dock ? `Cycle hire at ${dock.name}` : 'Cycle-hire day summary'}>
        ...
        {!missingDock && <p>{number(active.length)} hires in progress{dock ? ' involving this dock' : ''}</p>}
