...
export function HeroCardDismiss({ name, dismissed, onToggle }: { name: string; dismissed: boolean; onToggle: () => void }) {
  return <button className="london-hero-dismiss ms-control ms-panel-dismiss" type="button" aria-label={`${dismissed ? 'Show' : 'Close'} ${name} card`} aria-expanded={!dismissed} onClick={onToggle}>
