...
export function LondonNationalRailBoard({ snapshot, stations, stationId, time, windowStart, windowEnd, selectedId, onStation, onSelect, onSeek, onPulse, partial, animate, passengerContent, dismissed, dismissControl, boardContent }: {
  ...
  return <section className="london-national-rail-board" aria-label={`National Rail at ${station.name}`} data-hero-dismissed={dismissed}>
    ...
    <details open={expanded} onToggle={event => setExpanded(event.currentTarget.open)}>
      <summary><span>{boardContent ? 'TfL + National Rail' : 'National Rail'}</span><strong>{station.name}</strong></summary>
      ...
      {!boardContent && <button className="london-rail-pulse-link" type="button" onClick={onPulse}>Open {station.name} pulse</button>}
