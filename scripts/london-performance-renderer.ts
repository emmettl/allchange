...
export function londonPerformanceRenderer(): Plugin {
  return {
    name: 'london-performance', enforce: 'pre',
    transform(source, id) {
      ...
      if (!moduleId.endsWith('/@motionstudies/three/air-labels.js') &&
          ...
          !moduleId.endsWith('/@motionstudies/three/RoadTrafficLayer.js') &&
          !moduleId.endsWith('/@motionstudies/three/AirTrafficLayer.js')) return
      let code = source
      const replace = (before: string, after: string) => {
        if (code.split(before).length !== 2) throw new Error(`London performance hook needs review: ${before}`)
      ...
      } else if (moduleId.endsWith('/RoadTrafficLayer.js')) {
