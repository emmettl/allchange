...
export function londonNationalRailRenderer(): Plugin {
  return {
    ...
    transform(source, id) {
      ...
      return { code: 'import { lazy as railLazy, Suspense as RailSuspense } from "react";\nconst LondonNationalRailLayer = railLazy(() => import("/src/studies/LondonNationalRailLayer.tsx").then(module => ({ default: module.LondonNationalRailLayer })));\nimport { LondonQuietMap } from "/src/studies/LondonQuietMap.tsx";\n' + source.replace(hook,
        'props.quietMap && _jsx(LondonQuietMap, { projection, isPlaying: props.isPlaying, diagramSnapshot: props.quietDiagramSnapshot, spatialLayout: props.spatialLayout, spatialLayoutMix: props.spatialLayoutMix }), ' +
        'props.nationalRailSnapshot && props.boundary && (props.spatialLayoutMix ?? 0) === 0 && _jsx(RailSuspense, { fallback: null, children: _jsx(LondonNationalRailLayer, { snapshot: props.nationalRailSnapshot, boundary: props.boundary, projection, time: props.time, isPlaying: props.isPlaying, playbackRate: props.playbackRate, windowStart: props.snapshot.metadata.windowStart, windowEnd: props.snapshot.metadata.windowEnd, selectedId: props.nationalRailSelectedId, subdued: Boolean(props.selectedTrain || props.selectedRoute || props.selectedStation || props.selectedCategory || props.airCategorySelected || props.roadCategorySelected) }) }), ' + hook), map: null }
    },
  }
}
