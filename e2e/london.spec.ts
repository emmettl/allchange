...
test('vehicle count follows enabled air and road layers without a legend selection', async ({ page }) => {
  ...
  const railCount = await readCount()
  expect(railCount).toBeGreaterThan(0)

  await page.getByRole('button', { name: 'Show observed aircraft' }).click()
  const air = page.getByRole('button', { name: 'AIR', exact: true })
  await air.click()
  await expect.poll(readCount).toBeGreaterThan(0)
  const aircraftCount = await readCount()
  await air.click()
  await expect.poll(readCount).toBe(railCount + aircraftCount)
  await expect(status).toContainText('vehicles in motion')

  await page.getByRole('button', { name: 'Hide TfL rail' }).click()
  await expect.poll(readCount).toBe(aircraftCount)
  await expect(status).toContainText('aircraft observed')
  await expect(status).toContainText('Observed London airspace')
  await expect(status).not.toContainText('scheduled journeys')

  await page.getByRole('button', { name: 'Show reconstructed motorway traffic' }).click()
  const road = page.getByRole('button', { name: 'ROAD', exact: true })
  await road.click()
  await expect.poll(readCount).toBeGreaterThan(0)
  const roadCount = await readCount()
  await road.click()
  await expect.poll(readCount).toBe(aircraftCount + roadCount)
  await expect(status).toContainText('vehicles in motion')

  await page.getByRole('button', { name: 'Hide observed aircraft' }).click()
  await expect.poll(readCount).toBe(roadCount)
  await expect(status).toContainText('vehicles reconstructed')
  await expect(status).toContainText('London motorway flow')
  await expect(status).not.toContainText('scheduled journeys')
...
for (const time of [27900, 66600]) {
  test(`vehicle count covers all 64 layer combinations at ${time}`, async ({ page }) => {
