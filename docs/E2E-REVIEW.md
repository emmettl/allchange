...
Browser timings use the same local Mac, both projects, two workers and a prebuilt site. They measure the functional test command, not the complete hosted build/deployment pipeline. CI already builds once, runs one worker on each browser runner, and waits for the isolated WebKit cadence check. The local cadence command now targets WebKit explicitly to avoid booting a desktop case just to skip it.
